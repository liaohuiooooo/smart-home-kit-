...
{	 
  ...
  else
  {
    ...
    {
      wjq_log(LOG_INFO, "Hub is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_CDCC)
    {
      wjq_log(LOG_INFO, "Communications and CDC Control device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_HID)
    {
      wjq_log(LOG_INFO, "HID device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_PRINTER)
    {
      wjq_log(LOG_INFO, "Printer device is not supported.\n");
    }
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_SMARTCARD)
    {
      wjq_log(LOG_INFO, "Smart Card device is not supported.\n");
    }
    
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_VIDEO)
    {
      wjq_log(LOG_INFO, "Video device  is not supported.\n");
    }
    
    
    else if (pphost->device_prop.Itf_Desc[0].bInterfaceClass == USB_AVD)
    {
      wjq_log(LOG_INFO, "Audio/Video Devices is not supported.\n");
    }
    
    else
    {
      wjq_log(LOG_INFO, "The attached device is not supported. \n");
    }
