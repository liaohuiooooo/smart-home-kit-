...
{
	memset(Tx_String,0,Str_Len); //清空发送缓冲
...
{
	user_main_debug("进入MQTT_CONNECT");
...
{    
	...
	MQTT_TxLen += topiclen;
	user_main_debug("MQTT_TX_STRING,%s",Tx_String);
	if(whether)
	{
...
{  
	...
	Tx_String[MQTT_TxLen++] = BYTE1(topicLength);//主题长度MSB
	Tx_String[MQTT_TxLen++] = BYTE0(topicLength);//主题长度LSB 
	memcpy(&Tx_String[MQTT_TxLen],topic,topicLength);//拷贝主题
...
void MQTT_CONNECTING(void)
{
	...
	{
		user_main_info("CONNECTED TO SERVER!\r\n");
	}
	else 
	{
		user_main_info("NOT CONNECTED TO SERVER!");
		return;
	}
	
	if(MQTT_Connect(MQTT_CLIENTID, MQTT_USARNAME, MQTT_PASSWD) != 0)
	{
		user_main_info("MQTT SERVER SUCCESSFULLY CONNECTED！\r\n");
	}
	else
	{
		user_main_info("MQTT SERVER FAILED TO BE CONNECTED!");
		return;
	}

	if(MQTT_SubscribeTopic(MQTT_SUBSCRIBE_TOPIC,0,1) != 0)
	{
		user_main_info("MQTT subscribe");
