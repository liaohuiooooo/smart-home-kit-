...
{
	ESP8266_ATSendString("+++");TIM1_Delay_ms(50);
	ESP8266_ATSendString("+++");TIM1_Delay_ms(50);	
}
...
uint8_t ESP8266_Check(void)
{
	...
	while(check_cnt--)
	{
		...
		{
			user_main_debug("ESP 01S FOUND.");
			Clear_Rx(Rx_String);
			return 1;
		}
		TIM1_Delay_ms(200);
	}
	user_main_debug("ESP 01S NOT FOUN. ");
	return 0;
}
...
uint8_t ESP8266_Init(void)
{
	...
	{
		user_main_debug("ESP8266未连接到MCU.");//这里遇到了问题是，ESPCHECH无法检测到UART4_IT数据，通过修改NVIC的中断级别，便可解决；
		return 0;
	}
	else{
		user_main_debug("ESP8266已经连接到MCU.");
	}
...
//	{
//		user_main_debug("关闭回显失败");
//		return 0;      
//	}
//	user_main_debug("关闭回显成功");
	return 1;                         //设置成功
}
